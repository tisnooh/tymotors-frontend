...
export default function ResetPassword() {
  ...
  if (done) return <main className="pt-32 pb-24 min-h-[70vh]"><div className="ty-container max-w-md"><h1 className="ty-display text-white text-4xl">Mot de passe modifié</h1><p className="text-emerald-200 mt-4">Votre mot de passe a été modifié.</p><Link className="ty-btn-primary w-full mt-6" to="/auth?mode=login">SE CONNECTER</Link></div></main>;
  return <main className="pt-32 pb-24 min-h-[70vh]"><form className="ty-container max-w-md" onSubmit={async (e) => {
    ...
    <input required minLength={8} type="password" autoComplete="new-password" value={confirmation} onChange={(e) => setConfirmation(e.target.value)} className="mt-4 h-12 w-full rounded-xl border border-[#232B3A] bg-[#0F1115] px-4 text-white" placeholder="Confirmer le mot de passe" />
